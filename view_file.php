...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['status']) && isset($_POST['row_ids'])){
...
// Fetch file info
$res = $conn->query("SELECT filename FROM uploaded_files WHERE id=$fileId");
...
// Remove unwanted columns for display
foreach($rows as &$r){
    unset($r['file_id']); // do not show
}
...
<body>
<h2>Viewing: <?= htmlspecialchars($row['filename']) ?></h2>
